...
extension Notification.Name {
    ...
    /// D-02: Brücke für Menüpunkt "Verlauf…" → History-Window-Scene in SPRECHKRAFTApp.
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    /// GroqService — injiziert von SPRECHKRAFTApp für bessere Testbarkeit.
    ...
    /// Wird von SPRECHKRAFTApp.HiddenActivationView.onAppear aufgerufen,
    ...
    @objc private func openDocumentation() {
        if let url = URL(string: "https://github.com/mbieling/SPRECHKRAFT") {
            NSWorkspace.shared.open(url)
        }
    }

    @objc private func openSupport() {
        if let url = URL(string: "https://github.com/mbieling/SPRECHKRAFT/issues") {
            NSWorkspace.shared.open(url)
        }
    }
